...
function cashDrawerV2Render(){
  ...
  view.innerHTML=`<div class="page-heading compact"><div><p class="eyebrow">OPERATIONS / CASH DRAWER</p><h2>ลิ้นชักเก็บเงินทอน</h2><p class="muted">เงินทอนแยกจากใบแจ้งหนี้ · ตรวจนับตามชนิดเงินและประวัติกะ</p></div>${status}</div><div class="drawer-grid"><article class="panel drawer-status"><div class="drawer-hero"><span class="drawer-circle"><span class="material-symbols-outlined">payments</span></span><div><small>${shift?'กะที่กำลังดำเนินการ':'สถานะลิ้นชัก'}</small><h3>${esc(shift?.code||'ยังไม่มีรหัสกะ')}</h3><p>${shift?`เปิดโดย ${esc(shift.openedBy)} · ${esc(cashDrawerDateTime(shift.openedAt))}`:'เปิดกะด้วยชื่อผู้รับผิดชอบ'}</p></div></div>${shift?`<div class="drawer-stat-grid"><div><small>เงินตั้งต้น</small><strong>${ready?cashDrawerMoney(shift.openingCash):'ยังไม่กำหนด'}</strong></div><div><small>นำออกไปใช้</small><strong>${cashDrawerMoney(uses)}</strong></div><div><small>เงินคืนจากกะอื่น</small><strong>${cashDrawerMoney(returns)}</strong></div><div><small>ยอดที่ควรเหลือ</small><strong class="accent-text">${cashDrawerMoney(expected)}</strong></div></div><div class="drawer-opening-form" style="background:#fffcf9;border:1px solid #ebdccb;border-radius:10px;padding:14px;margin-top:16px;"><strong style="font-size:13px;color:#5a3820;">กำหนดเงินตั้งต้นของกะ</strong><small style="color:#7c695c;display:block;margin:2px 0 10px;">กำหนดจากหน้านี้หลังเปิดกะแล้ว</small><label class="drawer-field" style="display:grid;gap:6px;"><span style="font-size:11.5px;font-weight:700;color:#5a3820;">เงินตั้งต้น (บาท)</span><div class="drawer-opening-fields" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;"><input id="drawer-v2-opening-cash" type="number" min="0" step="0.01" value="${ready?Number(shift.openingCash):''}" placeholder="ยอดเงินตั้งต้น" style="flex:1;min-width:140px;height:42px;padding:8px 12px;border:1px solid #d4c4b5;border-radius:8px;"><button class="button button-soft" data-cash-drawer-v2="set-opening" style="height:42px;padding:0 16px;white-space:nowrap;border-radius:8px;">บันทึกเงินตั้งต้น</button></div></label></div><div class="drawer-close-auth" style="margin-top:16px;"><strong style="font-size:13.5px;color:#5a3820;">ปิดกะลิ้นชักเก็บเงิน</strong><label class="drawer-field" style="display:grid;gap:6px;margin:8px 0 10px;"><span style="font-size:11.5px;font-weight:700;color:#5a3820;">ชื่อผู้ปิดกะ <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-close-name" placeholder="กรอกชื่อผู้ปิดกะ" required autocomplete="name" style="height:42px;padding:8px 12px;border:1px solid #d4c4b5;border-radius:8px;"></label><button class="button button-primary full-width" data-cash-drawer-v2="close" style="height:44px;border-radius:8px;"><span class="material-symbols-outlined">lock</span>ปิดกะ</button></div>`:`<div class="drawer-open-auth drawer-open-gate"><div class="drawer-gate-title"><span class="material-symbols-outlined">lock_open</span><div><strong>เปิดลิ้นชักเก็บเงิน</strong><small>กรอกชื่อผู้รับผิดชอบก่อนเปิดกะ</small></div></div><label class="drawer-field" style="display:grid;gap:6px;margin-bottom:12px;"><span style="font-size:12px;font-weight:700;color:#5a3820;">ชื่อผู้เปิดกะ <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-open-name" placeholder="ลงชื่อผู้รับผิดชอบ" required autocomplete="name" autofocus style="height:44px;padding:8px 12px;border:1px solid #d4c4b5;border-radius:8px;"></label><button class="button button-primary full-width" data-cash-drawer-v2="open" style="height:44px;border-radius:8px;"><span class="material-symbols-outlined">lock_open</span>เปิดกะและสร้างรหัสกะ</button></div>`}</article><article class="panel reconciliation"><div class="panel-heading"><div><span class="title-icon"><span class="material-symbols-outlined">fact_check</span></span><h3>ตรวจนับเงินในลิ้นชัก</h3></div><span class="status-chip ${!shift?'neutral':!ready?'warning':difference===0?'success':'warning'}">${!shift?'รอเปิดกะ':!ready?'รอเงินตั้งต้น':difference===0?'ยอดตรงกัน':'มีส่วนต่าง'}</span></div>${shift?`${!ready?'<div class="drawer-notice">กรุณากำหนดเงินตั้งต้นด้านซ้ายก่อนเริ่มนับเงิน</div>':''}<div class="reconcile-row"><span>ยอดที่ควรเหลือ</span><strong>${cashDrawerMoney(expected)}</strong></div><div class="cash-denomination-grid">${cashDrawerV2DenominationInputs(shift.counts,ready)}</div><div class="reconcile-row"><span>นับได้จริง</span><strong id="drawer-v2-counted-total">${cashDrawerMoney(counted)}</strong></div><div class="reconcile-row difference"><span>ส่วนต่าง</span><strong id="drawer-v2-difference" class="${difference===0?'positive-text':'warning-text'}">${difference<0?'-':''}${cashDrawerMoney(Math.abs(difference))}</strong></div><button class="button button-outline full-width" data-cash-drawer-v2="save-count" ${ready?'':'disabled'} style="height:42px;margin:8px 0 16px;border-radius:8px;">บันทึกผลการนับ</button><div class="drawer-use-form" style="background:#fffcf9;border:1px solid #ebdccb;border-radius:12px;padding:16px;margin-top:16px;"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span class="material-symbols-outlined" style="font-size:20px;color:#8a5433;">shopping_bag</span><strong style="font-size:13.5px;color:#4a2b16;">กรณีนำเงินออกไปใช้</strong></div><small style="color:#78695d;display:block;margin-bottom:12px;">ระบุเหตุผลและยอดเงิน ระบบจะหักจากยอดที่ควรเหลือ</small><div class="drawer-use-fields-clean" style="display:grid;grid-template-columns:1fr;gap:10px;"><label class="drawer-field" style="display:grid;gap:5px;"><span style="font-size:11.5px;font-weight:700;color:#5a3e2b;">เหตุผล / หมายเหตุ <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-use-note" type="text" placeholder="พิมพ์เหตุผล เช่น ซื้ออุปกรณ์สำนักงาน หรือสำรองจ่าย" style="width:100%;height:42px;padding:8px 12px;border:1px solid #d4c4b5;border-radius:8px;font-size:13px;box-sizing:border-box;"></label><div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;"><label class="drawer-field" style="flex:1;min-width:140px;display:grid;gap:5px;margin:0;"><span style="font-size:11.5px;font-weight:700;color:#5a3e2b;">ยอดเงิน (บาท) <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-use-amount" type="number" min="0" step="0.01" placeholder="0.00" style="width:100%;height:42px;padding:8px 12px;border:1px solid #d4c4b5;border-radius:8px;font-size:14px;font-weight:600;color:#8a5433;box-sizing:border-box;"></label><button type="button" class="button button-soft" data-cash-drawer-v2="add-use" ${ready?'':'disabled'} style="height:42px;padding:0 16px;display:inline-flex;align-items:center;justify-content:center;gap:5px;font-size:12.5px;font-weight:600;border-radius:8px;white-space:nowrap;cursor:pointer;"><span class="material-symbols-outlined" style="font-size:17px;">remove_circle_outline</span><span>บันทึกเงินที่นำออก</span></button></div></div>${(shift.cashUses||[]).length?`<div class="drawer-use-list">${shift.cashUses.map((item,index)=>`<div><span>${esc(item.note)}</span><strong>${cashDrawerMoney(item.amount)}</strong><button class="icon-button" data-cash-drawer-v2="remove-use" data-cash-use-index="${index}" aria-label="ลบรายการนำเงินออก"><span class="material-symbols-outlined">delete</span></button></div>`).join('')}</div>`:''}</div><div class="drawer-use-form drawer-return-form" style="background:#f6f9fc;border:1px solid #d3e2f0;border-radius:12px;padding:16px;margin-top:16px;"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span class="material-symbols-outlined" style="font-size:20px;color:#1e6091;">savings</span><strong style="font-size:13.5px;color:#184e77;">เงินคืนจากกะอื่น</strong></div><small style="color:#52796f;display:block;margin-bottom:12px;">เติมเงินกลับเข้าลิ้นชัก โดยระบุว่าคืนมาจากกะไหน</small><div class="drawer-return-fields-clean" style="display:grid;grid-template-columns:1fr;gap:10px;"><div style="display:grid;grid-template-columns:minmax(120px, 1fr) 1.5fr;gap:10px;"><label class="drawer-field" style="display:grid;gap:5px;"><span style="font-size:11.5px;font-weight:700;color:#184e77;">รหัสกะต้นทาง <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-return-shift" placeholder="SHIFT-..." style="width:100%;height:42px;padding:8px 12px;border:1px solid #b5cddf;border-radius:8px;font-size:13px;box-sizing:border-box;"></label><label class="drawer-field" style="display:grid;gap:5px;"><span style="font-size:11.5px;font-weight:700;color:#184e77;">หมายเหตุเพิ่มเติม</span><input id="drawer-v2-return-note" placeholder="รายละเอียดการคืนเงิน" style="width:100%;height:42px;padding:8px 12px;border:1px solid #b5cddf;border-radius:8px;font-size:13px;box-sizing:border-box;"></label></div><div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;"><label class="drawer-field" style="flex:1;min-width:140px;display:grid;gap:5px;margin:0;"><span style="font-size:11.5px;font-weight:700;color:#184e77;">ยอดเงินที่คืน (บาท) <b class="required-note" style="color:#c62828;">*</b></span><input id="drawer-v2-return-amount" type="number" min="0" step="0.01" placeholder="0.00" style="width:100%;height:42px;padding:8px 12px;border:1px solid #b5cddf;border-radius:8px;font-size:14px;font-weight:600;color:#184e77;box-sizing:border-box;"></label><button type="button" class="button button-primary" data-cash-drawer-v2="add-return" ${ready?'':'disabled'} style="height:42px;padding:0 16px;display:inline-flex;align-items:center;justify-content:center;gap:5px;font-size:12.5px;font-weight:600;border-radius:8px;background:linear-gradient(135deg,#1e6091,#184e77);border-color:#184e77;color:#fff;white-space:nowrap;cursor:pointer;"><span class="material-symbols-outlined" style="font-size:17px;">add_circle_outline</span><span>บันทึกเงินคืน</span></button></div></div>${(shift.cashReturns||[]).length?`<div class="drawer-use-list">${shift.cashReturns.map((item,index)=>`<div><span>คืนจาก ${esc(item.fromShift)}${item.note?` · ${esc(item.note)}`:''}</span><strong>${cashDrawerMoney(item.amount)}</strong><button class="icon-button" data-cash-drawer-v2="remove-return" data-cash-return-index="${index}" aria-label="ลบรายการเงินคืน"><span class="material-symbols-outlined">delete</span></button></div>`).join('')}</div>`:''}</div>`:'<div class="empty-state"><span class="material-symbols-outlined">point_of_sale</span><p>ยังไม่มีรอบให้ตรวจนับ</p><small>เปิดกะก่อน แล้วจึงนับเหรียญและธนบัตร</small></div>'}</article></div><article class="panel"><div class="panel-heading"><div><span class="title-icon"><span class="material-symbols-outlined">history</span></span><h3>ประวัติการเปิด–ปิดกะ</h3></div><span class="count-chip">${cashDrawerStore.history.length} รอบ</span></div>${cashDrawerStore.history.length?`<div class="table-wrap"><table class="cash-drawer-history"><thead><tr><th>รหัสกะ</th><th>ผู้เปิดกะ / เวลา</th><th>ผู้ปิดกะ / เวลา</th><th class="align-right">ยอดเริ่ม</th><th class="align-right">ยอดปิดจริง</th><th class="align-right">ส่วนต่าง</th><th></th></tr></thead><tbody>${cashDrawerStore.history.map((item,index)=>`<tr><td class="mono">${esc(item.code)}</td><td><strong>${esc(item.openedBy)}</strong><small class="table-subtext">${esc(cashDrawerDateTime(item.openedAt))}</small></td><td><strong>${esc(item.closedBy||'-')}</strong><small class="table-subtext">${esc(cashDrawerDateTime(item.closedAt))}</small></td><td class="align-right">${cashDrawerMoney(item.openingCash)}</td><td class="align-right">${cashDrawerMoney(item.countedTotal)}</td><td class="align-right ${Number(item.difference||0)===0?'positive-text':'warning-text'}">${Number(item.difference||0)<0?'-':''}${cashDrawerMoney(Math.abs(Number(item.difference||0)))}</td><td><button class="button button-danger action-small" data-cash-drawer-v2="delete-history" data-cash-history-index="${index}"><span class="material-symbols-outlined">delete</span>ลบ</button></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-state"><span class="material-symbols-outlined">history</span><p>ยังไม่มีประวัติการเปิด–ปิดกะ</p><small>ประวัติจะถูกบันทึกเมื่อปิดกะสำเร็จ</small></div>'}</article>`;
}
window.cashDrawerStore=cashDrawerStore;
function loadCashDrawerStore(){
  try{
