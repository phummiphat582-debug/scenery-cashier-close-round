...
const accommodationItems=[...DATA.accommodationItems.map(item=>{const villaName=Object.keys(villaBathTypes).find(name=>item.name.startsWith(`${name} `)||String(item.villa||'').startsWith(`${name} `))||(item.name.startsWith('Pre-Wedding')?'Pre-Wedding':null);const normalized={...item,name:cleanEnglishText(item.name),category:cleanEnglishText(item.category)};return villaName?{...normalized,label:villaName,name:villaName,category:cleanEnglishText(villaBathTypes[villaName]||'Accommodation'),villa:villaName}:normalized}),{name:'E-Voucher Dinner 800 Baht (22)',category:'Package',rate:800},{name:'E-Voucher Dinner 1200 Baht (22)',category:'Package',rate:1200}].filter((item,index,items)=>items.findIndex(other=>other.name===item.name)===index);
...
function wireEvents(){
  document.addEventListener('click',event=>{
    ...
    if(event.target.closest('[data-close-modal]')||event.target.classList.contains('modal-backdrop')){$('#modal-root').innerHTML='';return}
...
function openSettlementModal(){
  const s=invoiceSnapshot();
  const currentStaff=localStorage.getItem('scenery-last-login-email')||'Cashier';
...
async function finalizeInvoice(){
  const preparer=($('#settlement-preparer')?.value||'').trim()||localStorage.getItem('scenery-last-login-email')||'Cashier';
  ...
  const record={reference:s.reference,customer:s.customer,villa:s.villa,docDate:s.docDate,total:s.subtotal,discount:s.discount,deposit:s.deposit,pendingTotal:s.pendingTotal,pendingCollections:pendingCollections.filter(row=>row.amount>0),preparer,closedAt:new Date().toLocaleString('th-TH'),proof,lines:state.invoiceLines.map(l=>({...l})),payments:state.payments.map(p=>({...p}))};
