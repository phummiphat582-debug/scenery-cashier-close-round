...
function installFinalInvoiceRules(){
  ...
  function updateLineRowVisual(index){
    ...
    if(row){
      ...
      if(cell){
        cell.innerHTML=`<span>${invoiceMoney(net)}</span>${(discount||deposit)?`<small class="line-gross">เต็ม ${invoiceMoney(gross)}${discount?` · ลด -${invoiceMoney(discount)}`:''}${deposit?` · มัดจำ -${invoiceMoney(deposit)}`:''}</small>`:''}`;
      }
    }
  }
